...
def negative_sampling(speaker, chunk, anchor_positive_pairs, tmp_dictionary, negative_wav_ids, wavs_dict,
    ...
    if flag == 'train':
        ...
        for batch in range(num_batches):
            ...
            feats_anchor = np.zeros((batch_size,198,39))
            feats_positive = np.zeros((batch_size,198,39))
            feats_negative = np.zeros((batch_size,198,39))
            selected_idx = []
            while anc_pos_batch != [None]:
                pairs_batch = []
                negative_speaker = []
                negative_chunk = []
                negative_string = []
                ...
                for idx, pairs in enumerate(anc_pos_batch):
                    ...
                    for neg_idx, neg in enumerate(negative_wav_ids):
                        ...
                        for neg_chunk in range(int(len(os.listdir(dict_dir_negative_speaker)))):
                            ...
                            if neg in k:
                                negative_chunk.append(neg_chunk)
                                negative_string.append(neg)
                ...
                for idx, out in enumerate(output_batch):
                    if out > 0:
                        ...
                        triplets.append(str(speaker) + "#" + str(negative_speaker) + "#" +
                                        ...
                                        str(negative_string[idx]) + "#" + str(chunk) + "#" + str(negative_chunk[idx]) +
                                        '#' + str(out))

                not_selected_indices = [t for t in list(range(len(output_batch))) if t not in selected_idx]
                if not_selected_indices != [None]:
                    anc_pos_batch = [anchor_positive_pairs[x] for x in not_selected_indices]
                else:
                    anc_pos_batch = []
